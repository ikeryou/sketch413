...
export class Item extends MyDisplay {
  ...
  constructor(opt:any) {
    ...
    this._emoji.innerHTML = Util.randomArr('ABCDEFGHIKLMNOPRSTUVWXYZ'.split(''))
  ...
  protected _update():void {
    ...
    if(this._c % 1 == 0) {
      ...
      const meWidth = emojiSize.width * 1
      const meHeight = emojiSize.height * 1
      ...
      this._eye.forEach((eye, i) => {
        ...
        const b = Util.map(Math.sin(Util.radian(effect.ang)), 2, Func.val(20, 50), -1, 1) * (1 - this.effecVal)
        const y = Math.sin(Util.radian(effect.ang * -0.65)) * eyeHeight * Func.val(0.5, 0.25) * (1 - this.effecVal)
